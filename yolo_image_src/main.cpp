...
// to be filled with final image filenames
#define IMAGE_FN_ARRAY {"test_image_0.png", "test_image_1.png", "test_image_2.png", "test_image_8.png", "test_image_9.png"}
...
int main(int argc, char const *argv[]) {
  ...
  for(const auto& text : IMAGE_FN_ARRAY) {
    ...
    output_file.open(std::string("predictions/") + std::string("predictions_") + std::string(text) + std::string("_") + std::string(timestamp) + std::string(".txt"));
    output_file << "object" << ',' << "confidence" << ',' << "left" << ',' << "top" << ',' << "right" << ',' << "bottom" << std::endl;

    std::cout << text << std::endl;

    // read input image
    auto image = cv::imread(std::string("images/") + std::string(text), cv::IMREAD_COLOR);
    double original_width = image.size().width;
    double original_height = image.size().height;

    // Compute the ratio for resizing and size for padding
    // std::cout << "Original Width : " << image.size().width << std::endl;
    // std::cout << "Original Height : " << image.size().height << std::endl;
    double scale_x =
      ...
      for (size_t j = 0; j < l_h; j++) {
        for (size_t anchor_k = 0; anchor_k < n_anchors; anchor_k++) {
          ...
          if (max_score > SCORE_THRESHOLD) {
            ...
            float original_right = original_x + original_w;
            float original_bottom = original_y + original_h;

            // Translate to left, top, right, bottom in original image coordinates
            float left = (original_x - w_pad/2)/(NETWORK_INPUT_WIDTH - w_pad) * original_width;
            float top = (original_y - h_pad/2)/(NETWORK_INPUT_HEIGHT - h_pad) * original_height;
            float right = (original_right - w_pad/2)/(NETWORK_INPUT_WIDTH - w_pad) * original_width;
            float bottom = (original_bottom - h_pad/2)/(NETWORK_INPUT_HEIGHT - h_pad) * original_height;
            std::cout << "LTRB: " << left << ',' << top << ',' << right << ',' << bottom << std::endl;

            output_file << labels[max_ind] << ',' << std::to_string(max_score).substr(0, 5) << ',' << left << ',' << top << ',' << right << ',' << bottom << std::endl;
    ...
    std::string output_image_name = std::string("predictions/predictions_") + std::string(text) + std::string("_") + std::string(timestamp) + std::string(".png");
    bool check = cv::imwrite(output_image_name, image);

    if (check == false) {
      std::cout << "Unable to save: " << output_image_name << std::endl;
      return 1;
    }
